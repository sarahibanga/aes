...
// Rijndael Substitution Box
	const sbox: [u8;256] = [ 0x63, 0x7c, 0x77, 0x7b, 0xf2, 0x6b, 0x6f, 0xc5, 0x30, 0x01, 0x67, 0x2b, 0xfe,
...
impl AES {
    ...
    fn create_keys(&self, key: String, roundcnt: usize) -> String {
        ...
        let tep = i64::from_str_radix(gw3.get_mut(0).unwrap().as_ref(), 16).unwrap() ^ (rcon[roundcnt - 1]);
